...
String updateJsonUrl = "%update_url"; /* Passa qui l'url raw del tuo file update.json */
String appVersion = "1.0"; /* Questa è la versione attuale scritta nel codice */
...
html.append("  if(UPDATE_JSON_URL && UPDATE_JSON_URL !== '%update_url') { checkForUpdates(true); }");
html.append("};");
...
html.append("  if (!UPDATE_JSON_URL || UPDATE_JSON_URL === '%update_url') {");
html.append("    h += \"<div>⚠️ URL di aggiornamento non configurato in Tasker (%update_url).</div></div>\";");
...
html.append("  try {");
html.append("    let res = await fetch(UPDATE_JSON_URL + '?t=' + new Date().getTime());");
...
html.append("           statusDiv.innerHTML = \"<div><span class='bold' style='color:#dc2626;'>Nuova versione \" + data.version + \" disponibile!</span></div><br><div class='line'>\" + data.changelog + \"</div><button class='btn-submit btn-update' onclick='downloadUpdate()'>Scarica e Copia Codice</button>\";");
html.append("        }");
html.append("      } else {");
html.append("        var statusDiv = document.getElementById('updateStatus');");
...
html.append("    let res = await fetch(newCodeUrl + '?t=' + new Date().getTime());");
html.append("    if(res.ok) {");
...
html.append("      showToast('✅ Codice copiato! Apri l\\'azione in Tasker, seleziona tutto il testo vecchio e incollaci questo sopra.');");
...
activity.runOnUiThread(new Runnable() {
    run() { webView.loadDataWithBaseURL(null, html.toString(), "text/html", "UTF-8", null); }
});